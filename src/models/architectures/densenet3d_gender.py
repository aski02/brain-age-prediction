######################################################################################
### Partially taken from https://github.com/EIDOSLAB/contrastive-brain-age-prediction
######################################################################################
...
class SupRegDenseNetOld(nn.Module):
    """encoder + regressor"""
    def __init__(self,):
        super().__init__()
        self.encoder = densenet121()
        self.fc_gender = nn.Linear(self.encoder.num_features, 2)
        self.fc_age = nn.Linear(self.encoder.num_features, 1)

    def forward(self, x):
        feats = self.features(x)
        gender_output = self.fc_gender(feats)
        age_output = self.fc_age(feats)
...
class SupRegDenseNetDropout(nn.Module):
    """encoder + regressor"""
    def __init__(self,):
        super().__init__()
        self.encoder = densenet121()
