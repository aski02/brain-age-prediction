...
class SupRegResNetDropout(nn.Module):
    """encoder + regressor"""
    def __init__(self, name='resnet18'):
        ...
        self.fc1 = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(dim_in + 1, 256),
            nn.ReLU(),
            nn.Dropout(0.5)  # Additional dropout after ReLU
        )
    ...
    def forward(self, x, sex):
        ...
        feats = torch.cat((feats, sex), dim=1)
        feats = self.fc1(feats)
        return self.fc2(feats), feats
...
class SupRegResNet(nn.Module):
    ...
    def __init__(self, name='resnet18'):
        ...
        model_fun, dim_in = model_dict[name]
        self.encoder = model_fun()
        self.fc1 = nn.Linear(dim_in+1, 256)
...
class SupCEResNet(nn.Module):
    """encoder + classifier"""
    def __init__(self, n_classes, name='resnet18'):
        super().__init__()
        model_fun, dim_in = model_dict[name]
        self.encoder = model_fun()
