...
class CropImaged(MapTransform):
    ...
    def __call__(self, data):
        d = dict(data)
        for key in self.keys:
            d[key] = d[key][self.global_min_x:self.global_max_x+1,
                            self.global_min_y:self.global_max_y+1, self.global_min_z:self.global_max_z+1]
        return d
...
class BrainGenderDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        self.train_data, self.val_data = self.openbhb_dataset.get_data()
        ...
        self.test_val = pd.concat(
            [self.val_data, prisma_val, add_prisma_split], ignore_index=True)
        self.test_val_charite = pd.concat(
            [prisma_val, add_prisma_split], ignore_index=True)
        self.test_val_openbhb = self.val_data
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset = CacheDataset(data=train_files, transform=train_transforms,
                                          cache_rate=0.5, runtime_cache="processes", num_workers=4, copy_cache=False)
