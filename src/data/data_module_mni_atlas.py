...
class CropRegiond(MapTransform):
    def __init__(self, keys, region_id, atlas_threshold):
        super().__init__(keys)
        ...
        if self.atlas_threshold == 0:
            atlas_img = nib.load('../MNI-maxprob-thr0-1mm.nii.gz')
        elif self.atlas_threshold == 25:
            atlas_img = nib.load('../MNI-maxprob-thr25-1mm.nii.gz')
        elif self.atlas_threshold == 50:
            atlas_img = nib.load('../MNI-maxprob-thr50-1mm.nii.gz')
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            ...
            max_size = max(sub_image_data.shape[1:])

            pad_sizes = [(0, 0)]
            for dim in range(3):
                size = sub_image_data.shape[dim + 1]
                pad_before = (max_size - size) // 2
                pad_after = max_size - size - pad_before
                pad_sizes.append((pad_before, pad_after))
