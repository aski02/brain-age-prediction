...
class OpenBHB_Dataset:
    def __init__(self, train_csv_path, val_csv_path, test_csv_path, data_root_path):
    ...
    def process_data(self, data, subset):
        ...
        data['Loc'] = data['ID'].apply(lambda id: os.path.join(
            self.data_root_path,
            'OpenBHB',
            f"{subset}_quasiraw",
            f"sub-{int(id)}_preproc-quasiraw_T1w.npy"
        ))
        data.loc[data['Scanner'] == 2, 'Scanner'] = 71
        data.loc[data['Scanner'] == 1, 'Scanner'] = 72
        data = data.dropna(subset=['Loc', 'Age', 'Gender'])
        data['patient_id'] = data['ID'].apply(lambda id: f"ID_{int(id)}")
        return data
    ...
    def get_data(self):
        ...
        combined_data = pd.concat(
            [train_data, val_data, test_data], ignore_index=True)
    ...
    def split_test_set(self, data):
        age_bins = [(10, 15), (15, 20), (20, 25), (25, 30), (30, 35),
        ...
        for lower, upper in age_bins:
            ...
            if len(bin_data) >= samples_per_bin:
                ...
                raise ValueError(
                    f"Not enough samples in age bin {lower}-{upper}")
